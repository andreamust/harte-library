...
HARTE_LARK_GRAMMAR = open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "harte.lark")).read()
...
def semitones_to_pitchclass(root: str, semitones: Tuple[int]) -> Tuple[int]:
  ...
  pitchclass = tuple(map(lambda d: root_pitch.transpose(d).pitchClass, semitones))
  return pitchclass
...
def chord_to_pitchclass(chord: str) -> Tuple[int]:
  ...
  if len(parsed) > 0:
    ...
    degrees = degrees.difference(map(lambda d: d.replace("*", ""), to_remove))
